...
| `uuid` | UUID v4 | `f47ac10b-...` | Full UUID. Guarantees uniqueness without coordination. |
...
# UUID IDs (fully unique)
id:
  strategy: uuid
```

::: tip Avoiding ID Collisions
When multiple contributors create tasks on separate branches, `sequential` IDs can collide after merge. Use `prefixed` (with unique per-contributor prefixes), `random`, or `uuid` to avoid this. If collisions do occur, use `taskmd deduplicate` to resolve them.
:::
