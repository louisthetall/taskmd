...
- [ ] Run **without_skill** baseline: `claude -p "show me all my tasks"` (no skill loaded, no CLAUDE.md/TASKMD_SPEC.md/.taskmd.yaml)
- [ ] Save without_skill output to `benchmark/iteration-1/eval-1-list-tasks/without_skill/outputs/result.md`
- [ ] Run **with_skill** variant: `claude -p "show me all my tasks" --allowedTools "Bash,taskmd:list-tasks"` (skill loaded)
...
- [ ] Added 4 new list-tasks evals to `evals.json` (IDs 14-17): filter bugs, top 3 as JSON, not-done sorted, custom columns
- [ ] Ran all 5 evals (10 total runs) with proper control conditions (baseline has no CLAUDE.md, .taskmd.yaml, or TASKMD_SPEC.md)
...
- [ ] benchmark.json contains comparison deltas (pass_rate) — token/time deltas not available
